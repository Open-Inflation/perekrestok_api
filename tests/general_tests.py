...
async def test_qualifier(schemashot: SchemaShot):
    async with PerekrestokAPI(debug=False, timeout=10.0) as api:
        resp = await api.General.qualifier()
        schemashot.assert_match(resp, "qualifier")


@pytest.mark.asyncio
async def test_feedback_form(schemashot: SchemaShot):
    async with PerekrestokAPI(debug=False, timeout=10.0) as api:
        resp = await api.General.feedback_form()
        schemashot.assert_match(resp, "feedback_form")


@pytest.mark.asyncio
async def test_delivery_switcher(schemashot: SchemaShot):
    async with PerekrestokAPI(debug=False, timeout=10.0) as api:
        resp = await api.General.delivery_switcher()
        schemashot.assert_match(resp, "delivery_switcher")


@pytest.mark.asyncio
async def test_current_user(schemashot: SchemaShot):
    async with PerekrestokAPI(debug=False, timeout=10.0) as api:
        resp = await api.General.current_user()
        schemashot.assert_match(resp, "current_user")
