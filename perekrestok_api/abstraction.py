...
class CatalogFeedFilter:
    ...
    """ID категорий бывают 2 видов - главные и дочерние. Оба - просто числа."""
    ...
    """Исключает товары без отзывов.

    Требуется уточнение: без отзывов или без оценок.
    """
    ...
    """Фильтр особенностей продукта: страна изготовления, тип продукта, бренд и т.п.

    Для получения доступных особенностей используйте результат `Catalog.form`.
    Ключи для `add`/`remove` находятся в `key` на верхнем уровне массива.
    Внутри каждого элемента доступные значения перечислены в `enumList`.
    """
